...
def herencia(padres, madres):	
	hijo1 = [[0 for i in range(cromosomas)] for j in range (parejas)]
	hijo2 = [[0 for i in range(cromosomas)] for j in range (parejas)]
...
def mutacion(hijos):
	for i in range(parejas):
		if random.random() < 0.001: #el minimo, en caso de ser necesario se debe de agregar un poco mas (hasta 25%)
...
def ario(personas):
    return any(all(c == 0 for c in individuo) for individuo in personas)
